# This is a base service class to ease the service creation process. To see an example of how to use this have a
# look at [`service/stub-service.coffee`](stub-service.html).
define [
  # Jump to [`helper/radian-module-helper.coffee`](radian-module-helper.html) ☛
  'helper/radian-module-helper'
], (helper) ->
  class RadianService
    @register: (name, deps) ->
      helper.register helper.type.service, name, deps, @

    constructor: () ->
      helper.construct @, arguments